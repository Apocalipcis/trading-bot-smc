...
| Сервіс | Порт | Опис |
|--------|------|------|
| **smc-trading-bot** | 8001 | Консольний бот, WebSocket, CLI |
| **smc-web** | 8000 | Веб-інтерфейс, API, Dashboard |
...
# Запуск бектесту в контейнері
python docker-run.py --backtest ETHUSDT

# Запуск live trading в контейнері
python docker-run.py --live ETHUSDT

# Перегляд логів
docker-compose logs -f smc-trading-bot    # Trading bot
docker-compose logs -f smc-web            # Web interface
...
✅ **Крос-платформність** - працює на Windows, Linux, Mac  
✅ **Ізольованість** - немає конфліктів залежностей  
✅ **Простота розгортання** - один команда для запуску  
✅ **Масштабованість** - легко додавати нові сервіси  
✅ **Консистентність** - однакове середовище скрізь  
...
- Redis кешування для швидкості
- Моніторинг та метрики контейнерів